...
class _SalaRepeteState extends State<SalaRepete> {
  ...
  void entrarSala(Sala sala) {
    setState(() {
      sala.jogadores.add(Jogador('Jogador ${sala.jogadores.length + 1}'));
      salaAtual = sala;
    });
  }
...
class _JogoRepeteComSalaState extends State<JogoRepeteComSala> {
  ...
  Future<void> rolarDado() async {
    ...
    if (resposta.statusCode == 200) {
      ...
      setState(() {
        imagemDado =
            'https://89251fa1-c1cc-432f-a9c6-f40c20048c08-00-fbjdp3ey1vbj.spock.replit.dev/$imagem';
        if (valorDado == 5) {
  ...
  void proximoJogador() {
    setState(() {
      widget.sala.jogadores[jogadorAtual].pontos =
          pontos; // salva os pontos do jogador atual
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        Text('Jogador Atual: ${widget.sala.jogadores[jogadorAtual].nome}'),
        if (widget.sala.encerrada)
          Column(
            children: [
              Text(widget.sala.resultado!),
              ...widget.sala.jogadores
                  .map((jogador) =>
                      Text('${jogador.nome}: ${jogador.pontos} pontos'))
                  .toList(),
            ],
          ),
        if (!widget.sala.encerrada)
...
class Sala {
  ...
  Sala(this.nome, String jogadorInicial)
      : jogadores = [Jogador(jogadorInicial)],
        encerrada = false,
        resultado = null;
}
